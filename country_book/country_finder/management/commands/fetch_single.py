...
class Command(BaseCommand):
    help = 'Fetch all countries from REST Countries API and save data for a single country by CCA3 code'
    ...
    def handle(self, *args, **kwargs):
        ...
        # Find the country with the matching CCA3
        country_data = next((data for data in countries_data if data.get('cca3') == cca3), None)
        if not country_data:
